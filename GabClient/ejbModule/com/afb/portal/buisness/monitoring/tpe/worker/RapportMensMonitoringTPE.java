...
/**
 * RapportMensMonitoringTPE
 * @author Owner
 * @version 1.0
 */
@Stateless
@TransactionAttribute(TransactionAttributeType.REQUIRED)
public class RapportMensMonitoringTPE implements IRapportMensMonitoringTPE{
	...
	public void runMonitoring(){
		...
		try{
			...
			//System.out.println("Rapport Mensuel lastDay "+lastDay+"-----------day--------"+day);
	...
	public  void process(){
		...
		try{
			...
			param.put("tittre", "REPPORT MENSUEL DE MONITORING DES TPES");
			param.put("sousTittre", "DU "+format.format(DateUtils.addMonths(new Date(),-1))+" AU "+format.format(new Date()));
			String outFileName ="RapportMensMonitoringTPE"+mois+".pdf";
			...
			String subject = "Rapport Mensuel de Monitoring des TPEs du "+heure;
			String messageCorps ="Bonjour ci joint le Rapport de Monitoring des TPES du "+format.format(DateUtils.addMonths(new Date(),-1))+" au "+format.format(new Date())+" Merci ..";
		...
		try{
			...
			for(Atm value :Atms){
				...
				if(value.getTid() != null && !value.getTid().trim().isEmpty()){
					...
					/**
					String datedebut = formater.format(DateUtils.addMonths(curentDate,-1));
					...
					List<Incident> incis = ServiceStarted.repportManager.findIncident(value, typInci,DateUtils.addMonths(curentDate,-1),curentDate);
					if(!incis.isEmpty()){
